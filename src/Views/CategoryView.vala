...
public class ApplicationsMenu.Views.CategoryView : Gtk.EventBox {
    public signal void search_focus_request ();

    public MenuView view { get; construct; }
    ...
    public CategoryView (MenuView view) {
        Object (view: view);
    }
    ...
    construct {
        ...
        listbox.row_activated.connect ((row) => {
            Idle.add (() => {
                if (!dragging) {
                    ((ApplicationsMenu.Widgets.AppListRow) row).launch ();
        ...
        listbox.drag_begin.connect ((ctx) => {
            ...
            if (selected_row != null) {
                ...
                var drag_item = (ApplicationsMenu.Widgets.AppListRow) selected_row;
    ...
    private bool create_context_menu (Gdk.Event event) {
        var selected_row = (ApplicationsMenu.Widgets.AppListRow) listbox.get_selected_row ();

        var menu = new ApplicationsMenu.Widgets.AppContextMenu (selected_row.app_id, selected_row.desktop_path);
    ...
    public void setup_sidebar () {
        ...
        foreach (unowned Backend.App app in view.app_system.get_apps_by_name ()) {
            listbox.add (new ApplicationsMenu.Widgets.AppListRow (app.desktop_id, app.desktop_path));
        }
        listbox.show_all ();
    ...
    private bool filter_function (ApplicationsMenu.Widgets.AppListRow row) {
